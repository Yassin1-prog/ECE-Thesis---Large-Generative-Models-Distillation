...
probability distribution of a dataset so that novel, realistic samples may be
...
learning, and scientific simulation, among many other domains. This chapter
provides a systematic survey of the principal families of deep generative models,
tracing the historical arc from early probabilistic approaches to contemporary
score-based and flow-based methods.
...
Boltzmann machines \cite{smolensky1986information}, and deep Boltzmann machines
\cite{salakhutdinov2009deep}. However, the confluence of large-scale datasets,
modern GPU hardware, and expressive neural network architectures precipitated
a qualitative shift in the capability of generative systems beginning in the
early 2010s \cite{bengio2013representation}.

Contemporary deep generative models can be broadly taxonomised into four
principal families, illustrated schematically in
Figure~\ref{fig:generative_taxonomy}:
\begin{enumerate}
...
the mathematical foundations of each paradigm in turn. This exposition
serves a dual purpose: it situates the contribution of the present thesis
within the broader literature, and it establishes the formal notation required
to analyse knowledge distillation techniques applied to diffusion models
...
and data augmentation \cite{higgins2017betavae}. Third, the probabilistic
...
collapse} \cite{bowman2015generating}. Furthermore, optimising the ELBO
...
the expected reconstruction error \cite{larsen2016autoencoding}. Numerous
extensions have been proposed to mitigate these shortcomings, including
hierarchical VAEs \cite{vahdat2020nvae, child2020very} and the introduction
of normalising flow posteriors \cite{rezende2015variational}. Nevertheless,
as image generation benchmarks evolved, the centre of gravity in the field
shifted towards adversarial and, ultimately, diffusion-based approaches.
...
The years following the original GAN proposal witnessed a prolific expansion
of the framework. The Deep Convolutional GAN (DCGAN) \cite{radford2015unsupervised}
...
GAN (WGAN) \cite{arjovsky2017wasserstein} addressed this by replacing the
...
synthesis. The StyleGAN series \cite{karras2019style, karras2020analyzing,
karras2021alias} demonstrated state-of-the-art perceptual quality on
...
BigGAN \cite{brock2019large} pushed the Fréchet Inception Distance (FID)
...
cover $p_{\text{data}}$ in its entirety \cite{srivastava2017veegan}. The
...
\cite{salimans2016improved}, unrolled GANs \cite{metz2017unrolled} — offer
...
modelling via score matching, and the unifying perspective afforded by
...
convenience of this Markov structure is that the marginal $q(\mathbf{x}_t
...
of steps tends to infinity \cite{welling2011bayesian}. The generative modelling
...
original data distribution. This multi-scale approach anticipates the
continuous-time generalisation developed in the following subsection.
...
\cite{song2021denoising, lu2022dpmsolver}. This observation underpins a
broad family of inference-time acceleration methods, including DDIM
\cite{song2021denoising} and DPM-Solver \cite{lu2022dpmsolver}, which
...
\begin{figure}[ht]
    ...
    \caption{Illustration of the forward and reverse diffusion processes in
    the SDE framework. The forward SDE (top) gradually corrupts data
    $\mathbf{x}(0) \sim p_{\text{data}}$ into noise $\mathbf{x}(T) \sim p_T$
    over continuous time. The reverse SDE (bottom) and the probability flow
    ODE (dashed) recover samples by integrating backwards in time, guided by
    the learned score network $\mathbf{s}_\theta(\mathbf{x}(t), t)$.}
    \label{fig:sde_forward_reverse}
\end{figure}

The SDE framework thus provides three practically significant contributions.
First, it furnishes a rigorous theoretical apparatus — grounded in stochastic
calculus — that clarifies the relationship between previously disparate model
families. Second, it decouples the \emph{design} of the forward process
...
model can be expressed as a continuous trajectory in data space, a structure
that lends itself naturally to the application of teacher-student distillation:
a student model can be trained to reproduce the multi-step trajectory of a
pre-trained teacher in a single or small number of evaluations, dramatically
reducing inference cost without retraining from scratch.
...
\begin{figure}[ht]
    ...
    \caption{Illustration of conditional flow matching trajectories. Each trajectory
    interpolates linearly between a noise sample $\mathbf{x}_0 \sim p_0$ and a data
    sample $\mathbf{x}_1 \sim p_{\text{data}}$. The marginal flow (solid, red) is recovered
    by averaging over all conditional flows (dashed). Straight, non-crossing trajectories
    are characteristic of optimal-transport couplings.}
...
marginals $p_0$ and $p_1$. Under mild regularity conditions (Brenier's theorem
\cite{brenier1991polar}), $\pi^*$ is induced by a deterministic map $T^*$, i.e.\
...
A single reflow step—producing what the authors term a \emph{2-Rectified Flow}—already
achieves near-straight trajectories suitable for single-step generation after
distillation \cite{liu2022flow}. This connection between flow straightening and
distillation is significant: a distilled 2-Rectified Flow is conceptually analogous to
progressive distillation \cite{salimans2022progressive} applied to a flow-based model,
in that both seek to compress a multi-step sampler into a model that generates
high-quality samples in one or a small number of network evaluations. The relationship
between these approaches is examined in greater detail in
Chapter~\ref{chap:knowledge_distillation}.
...
\begin{figure}[ht]
    ...
    \caption{Schematic of the rectified flow reflow procedure. Left: an initial
    1-Rectified Flow trained on independent couplings exhibits crossing trajectories.
    Right: after a single reflow step (2-Rectified Flow), trajectories become
    approximately straight, enabling high-quality one-step generation after
    distillation. Adapted from \cite{liu2022flow}.}
...
and source-target coupling makes it highly amenable to distillation. The optimal
transport interpretation offers a principled criterion for minimizing inference cost,
while the rectified flow reflow procedure provides a practical iterative algorithm
for trajectory straightening. These properties position flow matching as both a
competitive generative modeling paradigm in its own right and a fertile ground for
the acceleration techniques surveyed in Chapter~\ref{chap:knowledge_distillation}.