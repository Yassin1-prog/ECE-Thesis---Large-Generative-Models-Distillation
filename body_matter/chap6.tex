...
The applicability of ViTKD \cite{yang2022vitkd} to diffusion model distillation is non-trivial,
but its core principles are well-motivated in this context. In JiT, the transformer blocks process
patch token sequences derived directly from noisy pixel-space images, and the resulting feature
representations are conditioned on both the class label $y$ and the continuous noise level $t$.
Early blocks encode coarse low-level denoising patterns, positional structure, and texture
statistics, while the final block aggregates global contextual information that determines the
model's final velocity prediction $v_\theta(\mathbf{z}, t, y)$. Supervising the student's
intermediate representations to match those of a larger teacher thus exerts pressure for the
student to develop more accurate and better-calibrated internal models of the denoising process,
which translates directly into higher generative quality.

A crucial distinction between the original ViTKD formulation and its present application,
however, concerns the treatment of individual training samples. In the classification setting for
which ViTKD was originally proposed, all training examples are drawn from a fixed image
distribution, and each sample contributes a comparably informative gradient signal to the
distillation objective. In the diffusion setting, by contrast, each training sample is a tuple
$(\mathbf{z}, t, y)$ in which the noisy input $\mathbf{z} = t\mathbf{x} + (1-t)\mathbf{e}$ is
parameterized by a continuous noise level $t \in [0, 1]$ drawn uniformly at each iteration. This
introduces a fundamental asymmetry in the informativeness of training samples that is entirely absent in the classification regime: at extreme noise levels, particularly for $t$ close to zero the
signal-to-noise ratio (SNR) of the noisy input is very low
(nearly pure noise), and the gradients produced by the distillation loss at these regimes tend to
be dominated by noise artifacts. To account for this, we adopt
\textbf{Min-SNR-$\gamma$ loss weighting} \cite{hang2023efficient}, which assigns per-sample weights
$w(t)$ based on the effective SNR of the training sample:
%
\begin{equation}
    \mathrm{SNR}(t) = \left( \frac{t}{1 - t} \right)^2, \qquad
    w(t) = \frac{\min\!\left(\mathrm{SNR}(t),\, \gamma\right)}{\gamma},
    \label{eq:snr_weight}
\end{equation}
%
where $\gamma > 0$ is a clipping hyperparameter that caps the maximum weight at unity, ensuring that $w(t) \in [0, 1]$ for all $t$. The assigned weights increase monotonically with $t$ concentrating the distillation signal on the high-timestep regime where teacher features are most informative. The $\gamma$ hyperparameter controls how much effect we want to allow low-mid timesteps to have on the distillation signal, increasing it causes the weights assigned to these timesteps to decrease while the opposite is true if we decrease $\gamma$. A balanced starting point is $\gamma = 0.55$.
When $\gamma = 0$, the weighting is disabled and all samples are treated
uniformly. This weighting is applied to both the mimicking and generation components of the ViTKDloss.

\subsection*{Shallow-Layer Mimicking Loss}
\label{subsec:vitkd_mimic}

The mimicking component of ViTKD operates on the output hidden states of the first two transformer blocks of both teacher and student. Let $\mathbf{F}_i^S \in \mathbb{R}^{B \times N \times D_S}$ and $\mathbf{F}_i^T \in \mathbb{R}^{B \times N \times D_T}$ denote the post-block feature tensors of the student and teacher at shallow layer $i \in \{0, 1\}$, respectively, where $B$ is the batch size, $N$ is the number of patch tokens, and $D_S$, $D_T$ are the respective embedding dimensions. When $D_S \neq D_T$, dimension alignment is required before computing the loss. The alignment is performed by a plain learned linear projection $\mathbf{W}_i \in \mathbb{R}^{D_T \times D_S}$. The aligned student feature is denoted $\hat{\mathbf{F}}_i^S \in \mathbb{R}^{B \times N \times D_T}$.
...
\end{equation}
%
where $\hat{\mathbf{H}}_b^S$ denotes the projector output for sample $b$, $\mathbf{m}_b \in \{0,1\}^{N \times 1}$ is the per-sample mask broadcast over the channel dimension, $\beta$ is a scalar loss weight, and the $\lambda$ factor in the denominator normalises for the varying number of masked tokens. The mask is sampled uniformly at random without replacement using a noise-based shuffling procedure. In practice, $\lambda = 0.5$ is used as the default masking ratio, meaning that half of the student's deep tokens are masked per step.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\linewidth]{figures/formula/AD.png}
    \caption{Illustration of the attention map distillation procedure. The student computes its own attention maps, with an additional
KL-divergence loss to distill patterns from the teacher during training. The teacher is no longer used
...
% ============================================================
\section{Weight Selection Initialization}
...
\paragraph{Class embedding remapping.} When the teacher was trained on a dataset with a different number of classes than the student's target (in our case the teacher was pretrained on ImageNet-1K while the student was trained on Imagenette which is a subset of Imagenet with only 10 classes), the class embedding table $\mathbf{E}_y \in \mathbb{R}^{(C+1) \times D}$ — where $C$ is the number of classes and the extra row corresponds to the unconditional null class — has incompatible first dimensions. In this case, we explicitly remap the teacher’s class-embedding table to the student label space by selecting only the rows corresponding to the target subset classes and then appending the teacher’s unconditional (null) embedding row

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.85\linewidth]{figures/formula/weight-selection.png}
    \caption{Weight selection. To initialize a smaller variant of a pretrained model, we uniformly select
...
The three components described in the preceding sections can be integrated into a single training procedure. Weight selection provides a one-time initialization of the student's parameters from the teacher's pretrained weights before training starts. During each subsequent training step, the student is optimized under a combined loss that aggregates the base denoising objective with the two distillation losses:
%
\begin{equation}
...
% ======================================================================
%  Figure: Distillation framework overview (architecture).
%  Requires the preamble block in distillation_diagrams_preamble.tex.
%  Place in chap6.tex; the label fixes the existing
%  \ref{fig:framework_overview} (currently rendering as ??).
% ======================================================================
\begin{figure}[ht]
    \centering
...
% ===== Shared noisy input (top centre) =====
\node[io,text width=48mm] (input) at (3.0,7.9)
  {Noisy image \;$\bm{z}=t\bm{x}+(1-t)\bm{e}$\\[1pt]
   conditioning $(t,\,y)$};

% ===== STUDENT column (left) =====
\node[sblk] (s_embed) at (0,6.0) {Linear Embed};
\node[sblk,below=of s_embed,shallow] (s_b1) {Block 1};
\node[sblk,below=of s_b1,shallow]   (s_b2) {Block 2};
\node[sblk,below=of s_b2]           (s_dots) {$\vdots$};
\node[sblk,below=of s_dots,shallow] (s_bL) {Block $L$};
\node[sblk,below=of s_bL]           (s_pred) {Linear Predict};
\node[font=\scriptsize,colorExperiment!80,below=2mm of s_pred] (s_v) {$\hat{\bm{v}}_S$};

% ===== TEACHER column (right) =====
\node[tblk] (t_embed) at (6.0,6.0) {Linear Embed};
\node[tblk,below=of t_embed,shallow] (t_b1) {Block 1};
\node[tblk,below=of t_b1,shallow]   (t_b2) {Block 2};
\node[tblk,below=of t_b2]           (t_dots) {$\vdots$};
\node[tblk,below=of t_dots,shallow] (t_bL) {Block $L$};
\node[tblk,below=of t_bL]           (t_pred) {Linear Predict};

% ===== column headers + locks (OUTER side, no feed crossing) =====
\node[font=\small\bfseries,colorExperiment,anchor=east,align=right]
  (s_hdr) at ($(s_embed.west)+(-6mm,1mm)$) {Student\\ JiT-S/16};
\node[font=\scriptsize,colorExperiment!70,anchor=north east,align=right]
  at ($(s_hdr.south east)+(0,-0.5mm)$) {33.3M\,$\cdot$\,trainable};
\lockopen{($(s_hdr.north east)+(2mm,1.5mm)$)}{colorExperiment}

\node[font=\small\bfseries,black!55,anchor=west,align=left]
  (t_hdr) at ($(t_embed.east)+(6mm,1mm)$) {Teacher\\ JiT-B/16};
\node[font=\scriptsize,black!45,anchor=north west,align=left]
  at ($(t_hdr.south west)+(0,-0.5mm)$) {131M\,$\cdot$\,frozen};
\lockclosed{($(t_hdr.north west)+(-2mm,1.5mm)$)}{black!55}

% ===== intra-column forward arrows =====
\foreach \a/\b in {s_embed/s_b1,s_b1/s_b2,s_b2/s_dots,s_dots/s_bL,s_bL/s_pred}
  \draw[sarr] (\a) -- (\b);
\foreach \a/\b in {t_embed/t_b1,t_b1/t_b2,t_b2/t_dots,t_dots/t_bL,t_bL/t_pred}
  \draw[farr] (\a) -- (\b);
\draw[sarr] (s_pred) -- (s_v);

% input feeds both columns (split low, below headers)
\draw[sarr] (input.south) -- ++(0,-3mm) -| (s_embed.north);
\draw[farr] (input.south) -- ++(0,-3mm) -| (t_embed.north);

% ===== weight selection (offline init): straight arrow in the embed-row gap =====
\draw[iarr] (t_embed.west) -- (s_embed.east);
\node[font=\scriptsize,initclr,align=center,fill=white,inner sep=1pt] at (3.0,6.42)
  {weight selection\\[-1pt]\itshape(offline init, once)};

% ===== distillation couplings (teacher -> student) =====
\draw[darr] (t_b1.west) -- (s_b1.east);
\draw[darr] (t_b2.west) -- (s_b2.east);
\node[font=\scriptsize,distill,fill=white,inner sep=1pt]
  at ($(s_b1.east)!0.5!(t_b1.west)+(0,3mm)$) {$\mathcal{L}_{\mathrm{mimic}}$};

\draw[darr] (t_bL.west) -- (s_bL.east);
\node[font=\scriptsize,distill,fill=white,inner sep=1pt]
  at ($(s_bL.east)!0.5!(t_bL.west)+(0,3.2mm)$) {$\mathcal{L}_{\mathrm{gen}},\ \mathcal{L}_{\mathrm{attn}}$};
\node[font=\scriptsize,distill!75,fill=white,inner sep=1pt]
  at ($(s_bL.east)!0.5!(t_bL.west)+(0,-3.2mm)$) {\itshape last $K$ blocks};

% ===== losses =====
\node[loss,below=12mm of s_v,text width=22mm] (lflow)
  {$\mathcal{L}_{\mathrm{flow}}=\|\hat{\bm{v}}_S-\bm{v}\|_2^2$};
\draw[sarr] (s_v) -- (lflow);

\node[loss,right=12mm of lflow,text width=46mm] (ltot)
  {$\mathcal{L}_{\mathrm{total}}=\mathcal{L}_{\mathrm{flow}}+\mathcal{L}_{\mathrm{ViTKD}}+\mathcal{L}_{\mathrm{attn}}$};
\draw[-{Latex[length=2mm]},distill,line width=0.8pt] (lflow) -- (ltot);

% backprop arrow to the student only (routed right of v_S)
\draw[sarr,dashed] (ltot.north) .. controls +(0,1.4) and +(1.6,-0.8) .. (s_pred.east);
\node[font=\scriptsize,colorExperiment!80] at ($(ltot.north)+(-0.1,0.95)$) {backprop (student only)};

% ===== inference inset =====
\begin{scope}[shift={(0.0,-3.9)}]
  \node[draw=black!25,rounded corners=3pt,fill=black!2,minimum width=72mm,
        minimum height=14mm,align=left,inner sep=4pt] (inf) {};
  \node[font=\scriptsize\bfseries,anchor=north west] at ($(inf.north west)+(1mm,-1mm)$) {Inference};
  \node[sblk,minimum height=5mm,text width=15mm,font=\scriptsize] (i_s)
        at ($(inf.center)+(-1.4,-2mm)$) {Student};
  \draw[sarr] ($(i_s.west)+(-8mm,0)$) node[left,font=\scriptsize]{$\bm{z}$} -- (i_s.west);
  \draw[sarr] (i_s.east) -- ++(8mm,0) node[right,font=\scriptsize]{$\hat{\bm{x}}$};
  \node[tblk,minimum height=5mm,text width=15mm,font=\scriptsize,draw=black!30,text=black!35]
        (i_t) at ($(inf.center)+(2.3,-2mm)$) {Teacher};
  \draw[red!55,line width=0.8pt] (i_t.south west) -- (i_t.north east);
  \draw[red!55,line width=0.8pt] (i_t.north west) -- (i_t.south east);
\end{scope}

\end{tikzpicture}
    }
    \caption[Overview of the proposed quality-boosting distillation framework.]{%
    Overview of the proposed teacher--student distillation framework. A single
    noisy image $\bm{z}=t\bm{x}+(1-t)\bm{e}$ with conditioning $(t,y)$ is fed to
    both the trainable student (JiT-S/16) and the frozen teacher (JiT-B/16). Three
    complementary signals transfer knowledge from teacher to student: shallow-layer
    feature \emph{mimicking} ($\mathcal{L}_{\mathrm{mimic}}$) on the first two blocks,
    deep-layer \emph{generation} together with \emph{attention} distillation
    ($\mathcal{L}_{\mathrm{gen}},\mathcal{L}_{\mathrm{attn}}$) on the last $K$ blocks,
    and a one-time \emph{weight-selection} initialisation copied from the teacher
    before training. Only the student receives gradients from the combined objective
    $\mathcal{L}_{\mathrm{total}}=\mathcal{L}_{\mathrm{flow}}+\mathcal{L}_{\mathrm{ViTKD}}
    +\mathcal{L}_{\mathrm{attn}}$; at inference the teacher is discarded and the student
    runs as a standalone model at unchanged cost.}
